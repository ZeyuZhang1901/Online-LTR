...
def average_ndcg_at_k(dataset, query_result_list, k, count_bad_query=False):
    ...
    for query in dataset.get_all_querys():
        ...
        #     continue
        if len(dataset.get_relevance_docids_by_query(
                query)) == 0:  # no docs in this query
            if count_bad_query:
                num_query += 1
            continue
        else:
            pos_docid_set = set(dataset.get_relevance_docids_by_query(query))
        dcg = 0.0  # dcg@k of list generated by the ranker
        ...
        for docid in pos_docid_set:
            rel_set.append(
                dataset.get_relevance_label_by_query_and_docid(query, docid))
        rel_set = sorted(rel_set, reverse=True)  # from top to bottom
        n = len(pos_docid_set) if len(pos_docid_set) < k else k
        idcg = 0  # dcg@k of an ideal list ordered by relevance of each doc
        for i in range(n):
            idcg += ((2**rel_set[i] - 1) / np.log2(i + 2))

        ndcg += (dcg / idcg)  # normalize
        num_query += 1
