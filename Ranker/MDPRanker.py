...
class MDPRanker(AbstractRanker):
    ...
    def __init__(self,
        ...
        W1 = tf.Variable(
            tf.truncated_normal([self.Nfeature, 1],
                                stddev=0.1 / np.sqrt(float(Nfeature))))
    ...
    def TFupdate_policy_trust(self, query, ranklist, rewards, dataset):
        ...
        for pos in range(lenghth):
            loss, _ = sess.run(
                ...
                feed_dict={
                    input_docs: feature_matrix[ranklist],
                    position: [0],
                    learning_rate: self.lr * rewards[pos]
                })
            ranklist = np.delete(ranklist, 0)
    ...
    def TFupdate_reward_trust(self, query, ranklist, rewards, dataset):
        ...
        for pos in range(lenghth):
            if rewards[pos] > 0:
                loss, _ = sess.run(
                    [cross_entropy_pos, train_step_pos],
                    feed_dict={
                        input_docs: feature_matrix[ranklist],
                        position: [0],
                        doc_length: len(ranklist),
                ...
                loss, _ = sess.run(
                    [cross_entropy_neg, train_step_neg],
                    feed_dict={
                        input_docs: feature_matrix[ranklist],
                        position: [0],
                        doc_length: len(ranklist),
    ...
    def get_scores(self, features):
        ...
        result = sess.run([scores], feed_dict={input_docs:
